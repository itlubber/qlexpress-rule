...
控制台右侧的“主题设置”入口提供白天/夜间模式、8 个固定纯色色卡、4 个固定渐变色卡、深浅侧栏、流式/定宽内容区、固定侧栏和色弱模式。修改配置会立即预览；只有点击“保存设置”才会持久化，点击取消、关闭按钮、遮罩或按 Esc 都会恢复打开抽屉前的配置。“恢复默认”同样只更新预览，需再次保存才会生效。

启用控制台登录时，主题写入 `console_user_preference`，以当前 Session 中的 `console_user.id` 隔离，刷新、重新登录或更换设备后都能恢复；接口不接受客户端指定用户 ID。关闭登录或尚未登录时，配置降级保存到当前浏览器的 `localStorage`。服务端配置是登录场景的最终权威，本地镜像只用于页面加载时尽早应用主题并减少闪烁。

白天与夜间模式由统一语义令牌驱动，覆盖布局、业务页面、Element Plus 浮层与控件、9 类设计器和 Monaco 编辑器。渐变预设用于主按钮、品牌装饰和重点选中态；文字、边框和焦点环使用同一预设的实色主色与派生色阶，避免把任意 CSS 写入用户配置。
